class Spirit extends Base {
	...
	init(f_left, f_top, direction) {
		this.f_left = f_left;
		this.f_top = f_top;
		this.f_height = Util.imgObj[ this.states[ this.states.default ].bg ].height;
		this.direction = direction;
		this.frames = this.spiritFrames();
		this.shadow = this.Shadow();
		this.animate = this.Animate();
		this.queue = this.Queue();
		this.lock = this.Lock();
		this.movelock = this.Lock();
		this.keyManage = this.KeyManage(this.config.keyMap);
		this.collision = this.Collision();  //类型, 可推动, 可销毁
		this.attack = new Fighter(this).init();
		this.waveBoxing = new WaveBoxing(this).init();
		this.statusManage = this.StatusManage();
		this.stageManage = this.Stage();
		this.event();
	...
	changeBg(bg, framesNum, combo, position) {
...
class Fighter extends Base {
	...
	init(){
		...
		this.attackEffect = this.AttackEffect();
	...
	start( left, top, easing, effect_position, sound ){
	...
	stickStart( easing, effect_position, sound ){
		var self = this;
		this.easing = easing;
		this.effect_position = effect_position;
		this.sound = sound;
...
class WaveBoxing extends Fighter {
	constructor(master) {
		super();
		this.width = 56;
		this.height = 32;
	...
	init() {
		...
		this.frames = this.spiritFrames();
		this.animate = this.Animate();
		this.collision = this.Collision(48, 32);
		this.attackEffect = this.AttackEffect(this);
